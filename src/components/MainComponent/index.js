...
class MainComponent extends Component {
    ...
    activeTabFn = d => {
        console.log('hi')
        this.setState({ activeTab: d })
    }

    checkingFn = url => {
        const { image, sec } = this.state
        console.log('url', url)
        ...
        if (image === url) {
            this.setState(p => ({
                score: p.score + 1,
                image:
                    imagesList[Math.floor(Math.random() * 1 * imagesList.length)]
                        .imageUrl,
            }))
        } else if (image !== url || sec < 0) {
    ...
    render() {
        ...
        const { activeTab, image, score, isGameOver, sec, id } = this.state
        console.log('image', image)

        // console.log(
        //   'filteredList',
        //   imagesList.filter(m => activeTab === m.category).map(p=>
        // )
        ...
        return (
            <div>
                ...
                <main className="main">
                    <img src={image} alt="match" className="match-img" />
                    <ul className="tab-item">
                        {tabsList.map(p => (
                            <TabItem
                                key={p.tabId}
                                p={p}
                                isTrue={activeTab === p.tabId}
                                activeTabFn={this.activeTabFn}
                            />
                        ))}
                    </ul>
                    <ul className="image-tab-container">
                        {imagesList
                            .filter(m => activeTab === m.category)
                            .map(p => (
                                <ImageItem
                                    image={p}
                                    key={p.imageUrl}
                                    checkingFn={this.checkingFn}
                                />
                            ))}
                    </ul>
                </main>
